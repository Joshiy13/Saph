...
class TicTacToeCog(commands.Cog):
    ...
    @commands.slash_command(name="tictactoe", description="Play Tic Tac Toe")
    async def tictactoe(self, ctx):
        guild_id = ctx.guild_id
        if guild_id in self.active_games:
            await ctx.respond("There is already an active game in progress.")
            return
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        if isinstance(interaction, discord.MessageInteraction):
            # Cleanup the game when the message is deleted
            if interaction.type == discord.InteractionType.message_delete:
                self.cleanup_game(interaction.guild_id)
        elif interaction.type == discord.InteractionType.component:
            # Cleanup the game when the view is closed or timed out
            if interaction.data.get("custom_id") == "stop_tictactoe":
                self.cleanup_game(interaction.guild_id)
